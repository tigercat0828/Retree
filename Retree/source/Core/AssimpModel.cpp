...
// constructor, expects a filepath to a 3D model. load with filename/filename.obj
AssimpModel::AssimpModel(string const& filename, bool gamma) : gammaCorrection(gamma) {
    
    fs::path path = fs::current_path() / "Assets" / "Models" / filename;
...
void AssimpModel::loadModel(string const& path) {
    ...
    {
        Logger::Error("Assmip error : {}", importer.GetErrorString());
        return;
    }
...
vector<AssimpTexture> AssimpModel::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName) {
    vector<AssimpTexture> textures;
